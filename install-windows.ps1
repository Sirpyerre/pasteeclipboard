...
Write-Host "[INFO] Pastee Clipboard - Windows Installation" -ForegroundColor Cyan
...
if (-not $isAdmin) {
    Write-Host "[WARNING] Warning: Not running as Administrator. Some features may not work." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "[SUCCESS] Go is installed: $goVersion" -ForegroundColor Green
} catch {
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[INFO] make is available" -ForegroundColor Green
    }
} catch {
    Write-Host "[WARNING] make is not available. Using direct go build..." -ForegroundColor Yellow
    $useMake = $false
}

Write-Host ""
Write-Host "[INFO] Building Pastee Clipboard..." -ForegroundColor Cyan
...
try {
    ...
    if (Test-Path "bin/pastee.exe") {
        ...
        Write-Host ""
        Write-Host "[INFO] To run Pastee Clipboard:" -ForegroundColor Cyan
        Write-Host "   .\bin\pastee.exe" -ForegroundColor White
        Write-Host ""
        Write-Host "[INFO] To add to Windows Startup:" -ForegroundColor Cyan
        ...
        if ($run -eq "Y" -or $run -eq "y") {
            Write-Host "[INFO] Starting Pastee Clipboard..." -ForegroundColor Cyan
            Start-Process -FilePath "bin\pastee.exe"
            Write-Host "[SUCCESS] Pastee is running! Look for the icon in the system tray." -ForegroundColor Green
        }
    } else {
